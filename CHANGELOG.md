...
- `kdx diagnose DEPLOYMENT` command — connects to the current kubeconfig context and returns a root-cause diagnosis
- Four failure classes: `CrashLoopBackOff`, `OOMKilled`, `ImagePullBackOff`, `Pending`
- Deterministic pre-classifier grounds the model prompt before the API call
...
- 30-second timeout on model API calls with `529` (overloaded) error handling
